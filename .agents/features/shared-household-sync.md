...
- Membership: one household per account; owner-created invite links with expiry and revocation.
...
- [ ] Add a `src/lib/sync/` domain boundary for auth, remote storage, connectivity, mutation queue, and conflict resolution.
- [ ] Extend the Dexie schema with household identity, sync metadata, deleted-at tombstones, server version, last-synced time, and a durable outbox.
...
- [ ] Build create-household, sign-in, invite copy/revoke, invite acceptance, local-inventory migration, and sync-status/error UI.
- [ ] Add English and Polish translations for all household and sync flows.
...
- [ ] Test Dexie migrations and outbox ordering/idempotency/conflicts, Supabase RLS and sync boundaries, and two-device offline/reconnect scenarios.
