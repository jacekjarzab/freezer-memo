...
export function HouseholdSyncPanel({ t, syncStatus, syncNow }: { t: TFunction; syncStatus: SyncStatus; syncNow: (allowMigration?: boolean) => Promise<{ status: SyncStatus; reason?: string }> }) {
  ...
  const [members, setMembers] = useState<Array<{ userId: string; role: 'owner' | 'member'; email: string | null }>>([]);
  const [openMemberEmail, setOpenMemberEmail] = useState<string | null>(null);
  ...
  const createHousehold = async () => {
    ...
    try {
      ...
      await saveSyncMetadata({ householdId: result.id, migrationState: 'pending', cursor: null });
       setHouseholdId(result.id); setMembers(sessionUserId ? [{ userId: sessionUserId, role: 'owner', email: sessionEmail }] : []); setHouseholdName(''); setNotice({ key: 'account.householdCreated', error: false });
    } catch (error) { showError(error); }
  };
